...
# 
# Constraint(s): Minimize Marketing Cost
...
# - ML Success Criteria: Accuracy: NA; 
                         Performance/Speed: Complete processing within 5 mins on every quarterly data
# 
# - Economic Success Criteria: Increase the Retail Store profits by atleast 15%

HLD - DAR - DLD (Watch AI for Business Professionals Module)
...
# Description:
# A total of 9835 transactions data was captured for a specific month.
'''

# Code Modularity

# Mlxtend (Machine Learning extensions) is a Python library which is useful library for the regular data science tasks.

# pip install mlxtend

# Install the required packages if not available

# Import all required libraries & give comments against each line of code
...
X_1hot_fit = TE.fit(groceries_list_new)
...
X_1hot = X_1hot_fit1.transform(groceries_list_new)
...
pop_item.plot.barh() # Plot a horizontal bar chart for the most popular items
plt.title('Most popular items') # Set the title of the plot
plt.gca().invert_yaxis()  # gca means "get current axes" # Invert the y-axis to display items with the highest count at the top
...
# Database does not accept frozensets
...
# End of Code for Association Rules
